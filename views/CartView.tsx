...
const CartView: React.FC<CartViewProps> = ({ items, onRemove, onBack }) => {
  const handleCheckout = () => {
    ...
    const productList = items.map((item, idx) => `${idx + 1}. ${item.name} (${item.price})`).join('\n');
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-16">
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-4 mb-24">
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all">
          ...
          <div className="mt-12 p-6 bg-slate-900 rounded-3xl text-white">
            ...
            <button 
              ...
              className="w-full py-4 bg-slate-800 text-white rounded-2xl font-black text-sm flex items-center justify-center gap-3 hover:bg-slate-700 transition-all transform active:scale-95 uppercase tracking-widest shadow-xl shadow-slate-900/20"
            >
