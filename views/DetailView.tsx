      ...
      <div className="grid grid-cols-1 md:grid-cols-12">
        ...
        <div className="md:col-span-7 lg:col-span-8 md:sticky md:top-0 md:h-screen">
          <div className="relative aspect-square md:aspect-auto md:h-full">
            <button 
              onClick={onBack}
              className="absolute top-6 left-4 z-10 group inline-flex items-center gap-3 text-[10px] font-black text-slate-600 hover:text-slate-900 transition-all uppercase tracking-[0.25em]"
            >
        ...
        <div className="md:col-span-5 lg:col-span-4 flex flex-col px-6 pt-10 pb-20 sm:px-12 sm:py-16">
          <h1 className="text-2xl md:text-4xl font-black text-slate-900 mb-2 leading-tight tracking-tighter">
            {product.name}
          </h1>
          
          <div className="text-2xl md:text-3xl font-black text-slate-900 mb-6 tracking-tight">
            {product.price}
          </div>

          {/* Elegant Variant Selector */}
          <div className="mb-8">
            <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-widest mb-4">
              Varian Warna
              {!selectedVariant && <span className="ml-2 text-xs font-normal normal-case text-red-500 animate-pulse">(Pilih satu)</span>}
            </h3>
            <div className="flex flex-wrap items-center gap-3">
              {product.variants.map(variant => (
                <button
                  ...
                  className={`flex items-center gap-3 pl-2 pr-4 py-2 rounded-full border-2 transition-all duration-300 text-xs font-bold ${
                    selectedVariant?.id === variant.id 
                    ? 'bg-slate-900 text-white border-slate-900' 
                    : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100 hover:border-slate-300'
                  }`}
                  aria-label={`Pilih warna ${variant.colorName}`}
                >
                  <span 
                    className={`w-5 h-5 rounded-full block ${selectedVariant?.id === variant.id ? 'ring-2 ring-white/60' : ''}`}
          ...
          <div className="mb-8">
            <h3 className="text-[10px] font-black text-slate-900 uppercase tracking-widest mb-3 border-b border-slate-100 pb-1">Detail Produk</h3>
