...
const FloatingBottomNav: React.FC<FloatingBottomNavProps> = ({ onHomeClick, onAboutClick, activeProduct, activeVariant }) => {
  const handleWhatsAppClick = (product?: Product, variant?: ProductVariant) => {
    const phoneNumber = "6281234567890"; // Ganti dengan nomor asli
    const text = product && variant
      ? `Halo YMG Official Store, saya tertarik dengan produk: ${product.name} (Warna: ${variant.colorName}).`
      : "Halo YMG Official Store, saya ingin bertanya mengenai koleksi tas Anda.";
    ...
    return (
      <div className="fixed bottom-0 left-0 w-full z-50 md:hidden bg-white/95 backdrop-blur-xl border-t border-slate-100 shadow-[0_-5px_30px_rgba(0,0,0,0.05)]">
        <div className="p-4 pb-safe">
          <button
            onClick={() => activeVariant && handleWhatsAppClick(activeProduct, activeVariant)}
            disabled={!activeVariant}
            className="w-full py-4 bg-slate-900 text-white rounded-2xl text-sm font-bold uppercase tracking-widest hover:bg-slate-800 transition-colors transform active:scale-95 shadow-lg shadow-slate-400/50 disabled:bg-slate-400"
          >
            {activeVariant ? 'Beli Sekarang' : 'Pilih Varian'}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full z-50 md:hidden bg-white/95 backdrop-blur-xl border-t border-slate-100 shadow-[0_-5px_30px_rgba(0,0,0,0.05)]">
      <div className="flex items-center justify-around py-2 px-2 pb-safe">
        ...
        <button 
          onClick={onHomeClick}
          className="flex flex-col items-center justify-center flex-1 py-1 group active:scale-95 transition-transform"
          aria-label="Home"
        >
