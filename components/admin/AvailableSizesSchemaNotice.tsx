...
const AvailableSizesSchemaNotice: React.FC<{ onDismiss?: () => void }> = ({ onDismiss }) => (
  <div className="bg-yellow-50 border border-yellow-300 text-yellow-800 p-4 rounded-xl my-4 animate-view-enter">
    <div className="flex justify-between items-start">
        <div>
            <p className="font-bold text-lg">⚠️ Aksi Diperlukan: Aktifkan Fitur Pilihan Ukuran</p>
            <p className="text-sm mt-2 mb-4">
              Fitur baru untuk pilihan ukuran (cth: S, M, L) memerlukan kolom <strong>"availableSizes"</strong> dengan tipe <strong>array of text</strong> (<code>text[]</code>) di tabel <code>products</code> Anda, tetapi kolom tersebut tidak ditemukan.
            </p>
        </div>
        {onDismiss && (
    ...
    <div className="text-xs space-y-3 bg-yellow-100 p-3 rounded-md">
      <div>
          ...
          <pre className="mt-2 text-[10px] bg-yellow-200 text-yellow-900 p-2 rounded-md overflow-x-auto">
            <code>
{`-- Menambahkan kolom 'availableSizes' untuk pilihan ukuran produk
...
-- Komentar: Perintah ini aman dijalankan berkali-kali.`}
          ...
          <p className="mt-2 text-[10px] italic">Setelah menjalankan SQL, silakan refresh halaman ini. Fitur pilihan ukuran produk akan berfungsi.</p>
      </div>
    </div>
  </div>
);

export default AvailableSizesSchemaNotice;